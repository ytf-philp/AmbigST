...
from fairseq.data.audio.audio_utils import get_fbank, get_features_or_waveform
import torchaudio
from fairseq.data.audio.feature_transforms import CompositeAudioFeatureTransform
...
class SpeechToTextDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.dataset_transforms = CompositeAudioDatasetTransform.from_config_dict(
            self.data_cfg.get_dataset_transforms(split, is_train_split)
        )
    ...
    def check_tgt_lang_tag(self):
        if self.data_cfg.prepend_tgt_lang_tag:
            ...
            #print(all(t in self.tgt_dict for t in tgt_lang_tags))
    ...
    def _get_source_audio(self, index: Union[int, List[int]]) -> torch.Tensor:
        ...
        if _is_int_or_np_int(index):
            source = get_features_or_waveform(
                ...
                use_sample_rate=self.data_cfg.config["use_sample_rate"],
