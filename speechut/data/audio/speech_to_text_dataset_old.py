...
class SpeechToTextDataset(FairseqDataset):
    ...
    def load_indexed_dataset(self, text: str):
        ...
        for str in str_list:
            ...
            if(self.unit_tokenizer.indices.get(str) >=600):
                print("error")
        idx.append(self.unit_tokenizer.indices.get('</s>'))
    ...
    def __getitem__(self, index: int) -> SpeechToTextDatasetItem:
        ...
        audio = None
        audio = self._get_source_audio(indices if has_concat else index)
        ...
        if self.src_texts is not None:
            source = self.load_indexed_dataset(self.src_texts[index]).long()
    ...
    def collater(
        ...
        sources = [x.audio for x in samples]
        ...
        if self.tgt_texts is not None:
            ...
            prev_output_tgt_tokens = fairseq_data_utils.collate_tokens(
                [x.target for x in samples],
                self.tgt_dict.pad(),
                eos_idx=None,
                left_pad=False,
            ...
            prev_output_tgt_tokens = prev_output_tgt_tokens.index_select(0, order)
            tgt_ntokens = sum(x.target.size(0) for x in samples)
