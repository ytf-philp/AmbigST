...
class LabelSmoothedCrossEntropyWithContrastiveCriterion(LabelSmoothedCrossEntropyCriterion):
    def __init__(
        ...
        self.queue_k=queue_k
        print("build_without_Asr_Weight")
        print("build_contrastive_queue")
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if model.training:
            ...
            audio_seq_hidden, short_audio_len = self.get_sequence_hidden(model, sample, encoder_out,
                                                                     is_text=False,
                                                                     return_short_audio_len=True) # B x h
            ...
            if not self.queue_k % batch_size == 0:
                #print("*"*100)
                #print("queue: ",self.queue_k)
                #print("batchsize: ",batch_size)
                batch_size=batch_size-self.queue_k % batch_size
                audio_seq_hidden=audio_seq_hidden[:batch_size,:]
                text_seq_hidden=text_seq_hidden[:batch_size,:]
            model._dequeue_and_enqueue(keys=(audio_seq_hidden,text_seq_hidden))
    ...
    def get_sequence_hidden(self, model, sample, packed_encoder_out,
        ...
        if is_text:
            encoder_out, encoder_padding_mask = model.encoder.w2v_model.forward_text_embedding(
                sample["source"])
        else:
            encoder_out = packed_encoder_out["encoder_embedding"]
            encoder_padding_mask = packed_encoder_out["encoder_padding_mask"][0]
            short_audio_len = encoder_out.shape[0]
